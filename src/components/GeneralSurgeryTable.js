...
const GeneralSurgeryTable = () => {
  ...
  const fetchRecords = async (currentPage, id) => {
    setLoading(true);
    try {
      const response = await get(`/GeneralSurgery/list/${currentPage}/10`);
  ...
  return (
    <div className="family-table-container">
      <div className="header-section">
        <h3>Operation Notes Log</h3>

        <div className="button-bar">
          <button
            className="btn-case"
            onClick={() =>
              navigate(`/doctor/patients/general-surgery/${patientId}`)
            }
          >
            + Create a Case Note
          </button>
                    ...
                    <tr key={record.id}>
                      ...
                      <td>
                        <button
                          onClick={() => {
                            navigate(
                              `/doctor/patients/general-surgery/${patientId}/?treatmentId=${record.id}`
                            );
                          }}
                          className="icon-btn"
                        >
...
export default GeneralSurgeryTable;
