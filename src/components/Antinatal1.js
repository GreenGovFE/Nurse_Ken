...
const Antinatal = ({ patientData }) => {
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      // navigate(`/doctor/patients/patient-details/${patientId||patientData?.patientId}`);
  ...
  const fetchVisit = async () => {
    ...
    try {
      const response = await get(
        `/appointment/get-appointment-bypatientId/${patientId || patientData?.patientId}/`
      );
  ...
  return (
    <div
      className="consultation-container w-100"
