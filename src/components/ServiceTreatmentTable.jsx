...
import { FiEdit3, FiFile, FiVideo } from "react-icons/fi";
...
const ServiceTreatmentTable = ({}) => {
  ...
  const selectRecord = (record) => {
    ...
    localStorage.setItem('appointmentId',record?.appointmentId);
    setPatientId(record?.patient?.id);
    setPatientName(
      `${record?.patient?.firstName} ${record?.patient?.lastName}`
    );
    setDiagnosis(record?.diagnosis);
    navigate(`/antenatal-record/${record?.patient?.id}/?treatmentId=${record.id}`);
  };

  const selectRecordEdit = (record) => {
    console.log("record", record);
    setPatientInfo(record);
    setAppointmentId(record?.appointmentId);
    localStorage.setItem('appointmentId',record?.appointmentId);
    ...
    setDiagnosis(record?.diagnosis);
    navigate(
      `/antenatal-record/${record?.patient?.id}/?treatmentId=${record.id}&edit=true`
    );
  };
                    ...
                    {records.length > 0 ? (
                      records?.map((record) => (
                        <tr key={record.id}>
                          ...
                          <td>
                            {record.deliveryType || "Antenatal Consultation"}
                          </td>
