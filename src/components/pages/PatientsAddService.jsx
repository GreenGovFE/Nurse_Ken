...
function PatientsAddService() {
  ...
  const getAllAdmittedPatients = async (currentPage) => {
    ...
    try {
      let res = await get(
        `/patients/admitted-patients?pageNumber=${currentPage}&pageSize=10`
      );
  ...
  const getAppointmentPatients = async () => {
    ...
    try {
      let res = await get(
        `/Appointment/get-pending-appointments?pageIndex=${currentPage}&pageSize=${itemsPerPage}`
      );
      ...
      <>
        {nurseTypes !== "vital" && (
          <div className="flex flex-v-center space-between wrap  m-t-20">
            <h3 className="float-left">Patients Add Service</h3>
