...
function Personal({ setSelectedTab, hide }) {
  ...
  const patientClassOptions = [
    ...
    { label: "HMO", value: "hmo" },
    { label: "Retainership", value: "retainership" },
  ];

  // Determine patient class from schema
  function getPatientClassFromSchema(patient) {
    if (patient?.hasHmo) return "hmo";
    if (patient?.hasRetainership) return "retainership";
    return "private";
  }

  const [patientClass, setPatientClass] = useState(
    getPatientClassFromSchema(patientInfo)
  );
  ...
  // For radio buttons:
  // {patientClassOptions.map(opt => (
  //   <label key={opt.value}>
  //     <input
  //       type="radio"
  //       name="patientClass"
  //       value={opt.value}
  //       checked={patientClass === opt.value}
  //       onChange={e => setPatientClass(e.target.value)}
  //     />
  //     {opt.label}
  //   </label>
          ...
          <TagInputs
            ...
            onChange={(e) => {
              ...
              setPayload((prev) => ({
                ...prev,
                hasHmo: value === "hasHmo",
                hasRetainership: value === "hasRetainership",
                // If neither HMO nor Retainership, it's Private
              }));
            }}
            options={[
              { name: "Private", value: "private" },
              { name: "HMO", value: "hasHmo" },
              { name: "Retainership", value: "hasRetainership" },
            ]}
          />
        </div>
        <div className="col-4">
