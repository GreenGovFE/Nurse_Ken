...
function AddMoreTreatment({
  ...
  const addTreatment = async () => {
    ...
    const dateOfVisit = visit?.dateOfVisit
      ? new Date(visit?.dateOfVisit)?.toISOString()
    ...
    // Example of how to populate payload2 from current state
    const payload2 = {
      patientId: +patientId,
      medications: medications.map((med) => ({
        ...
        doctorPrescription: {
          ...
          administrationFrequency: med.administrationFrequency?.toString() || "",
          routeOfAdministration: med.routeOfAdministration?.toString() || "",
          duration: med.duration?.toString() || "",
          dosage: "", // If you have a dosage field, use it here
        },
        comment: "", // Add comment if available
      })),
    ...
    const res = await post(
        // `/patients/${patientId}/ServiceTreatment/${treatment?.id}/add-additional-prescription`,
      `/ServiceTreatment/additional-prescription/${treatment?.id}`,
      payload2
    ); // Send payload to the endpoint
    console.log(res);
    if (res.isSuccess) {
      toast.success(res.data.message||"Treatment added successfully");
      closeModal();
    } else {
