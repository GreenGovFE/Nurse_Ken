...
function AdministerMed({
  ...
  const administerMed = async () => {
    ...
    const data = {
      administeredBy: nurses.find((nurse) => nurse.value === payload.nurse)
        ?.name,
      treatmentId: 0,
  ...
  const handleTranscript = (transcript) => {
    setPayload((prevPayload) => ({
      ...prevPayload,
      otherNotes: prevPayload.otherNotes
        ? prevPayload.otherNotes + " " + transcript
        : transcript,
    }));
  };
  ...
  const fetchPrescriptionLogs = async () => {
    console.log(treatment);
      ...
      try {
        const res = await get(
          `/patients/prescription-log-by-service-treatment?serviceTreatmentId=${treatment.id}`
        );
        if (Array.isArray(res)) {
          setPrescriptionLogs(res);
        }
      } catch (err) {
  ...
  // Fetch prescription logs on mount
  useEffect(() => {
      ...
      <div className="modal-content">
        ...
        <p>
          ...
          <span style={{ fontWeight: "bold", color: "green" }}>
            {medication?.pharmacyInventory?.productName} {medication?.quantity}{" "}
            {drugStrengthUnits.find(
              (unit) => unit.id === medication?.drugStrengthUnit
            )?.name || "N/A"}
          </span>{" "}
        </p>
        <p>
          ...
          <span style={{ fontWeight: "bold", color: "green" }}>
            {
              routesOfAdministration?.find(
                (route) => route.id === medication?.routeOfAdministration
              )?.name
            }
          </span>
        </p>
        <p>
          ...
          <span
            style={{
              fontWeight: "bold",
              color:
                medication?.administrationFrequency === 1 ? "red" : "green",
            }}
          >
            {
              administrationFrequencies?.find(
                (freq) => freq.id === medication?.administrationFrequency
              )?.name
            }
          </span>
        </p>
        ...
        {prescriptionLogs.length > 0 && (
          <div className="m-t-40">
            <h4>Administered Medication Log</h4>
            <table className="bordered-table-2 m-t-10">
              ...
              <tbody>
                {prescriptionLogs.filter((log) => log.medicationId === medication.id).map((log) => (
                  <tr key={log.id}>
                    <td>
                      {log.administeredDate
                        ? new Date(log.administeredDate).toLocaleDateString()
                        : ""}
                    </td>
                    <td>{log.administeredTime}</td>
                    <td>{log.medicationName}</td>
                    <td>{log.volumeGiven}</td>
                    <td>{log.units}</td>
                    <td>{log.administeredBy}</td>
                    <td>{log.otherNotes}</td>
                  </tr>
                ))}
              </tbody>
            </table>
