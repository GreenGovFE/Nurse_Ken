    ...
    try {
      let res = await get(
        `/patients/Allnurse/${sessionStorage.getItem(
          "clinicId"
        )}?clinicId=${sessionStorage.getItem(
          "clinicId"
                ...
                <table className="bordered-table-2">
                    ...
                    <tbody className="white-bg view-det-pane">
                        <tr>
                            ...
                            <td style={{ minWidth: "100px" }}>
                                {moment(visit?.dateOfVisit).format("hh:mm A")}
                            </td>
                    ...
                    .map((doc, index) => (
                        <div key={index}>
                            <div className="text-green pointer flex flex-direction-v flex-h-center">
                                <RiFileDownloadFill
                                    onClick={() => downloadFile(doc?.docName)}
                                    size={20}
                                />
                                <span className="" onClick={() => downloadFile(doc?.docName)}>
                                    Download {doc?.docName}
                                </span>
                                {/* Check if the document is a PDF, otherwise display images or other files */}
                                {doc?.docPath?.endsWith(".pdf") ||
                                doc?.docPath?.endsWith(".tif") ||
                                doc?.docPath?.endsWith(".bmp") ||
                                doc?.docPath?.endsWith(".tiff") ? (
                                    <iframe
                                        ...
                                        frameBorder="0"
                                    />
                                ) : doc?.docPath?.endsWith(".png") ||
                                    doc?.docPath?.endsWith(".jpg") ||
                                    doc?.docPath?.endsWith(".jpeg") ? (
