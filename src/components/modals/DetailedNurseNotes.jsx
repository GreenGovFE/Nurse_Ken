...
function DetailedNurseNotes({ closeModal, treatment, doctors, nurses, reset }) {
  ...
  const getVital = async () => {
    try {
      const res = await get(
        `/patients/vital-by-appointmentId?appointmentId=${treatment?.appointmentId}&pageIndex=1&pageSize=100`
      );
      ...
      <div className="modal-content">
        <div className="flex space-between">
          <div className="flex flex-v-center m-t-20 m-l-10 col-6">
            <p className="bold-text m-r-10">Nurse Notes</p> |{" "}
            <p className="m-l-10">{patientName}</p>
          </div>
          <div className="flex flex-v-center m-t-20 m-l-10">
            <button
              onClick={() => setNurseNotesModal(true)}
              className="add-note"
            >
        ...
        <div>
          <table className="bordered-table-2 m-t-40">
            <thead className="border-top-none">
              <tr className="border-top-none">
                ...
                {/* Dosage header spanning two columns */}
              </tr>
              <tr className="">
                <th>Quantity</th>
                <th>Duration</th>
              ...
              <tr>
                ...
                <td>
                  {medications.map((item) => (
                    ...
                    >
                      <span>
                        {item?.doctorPrescription?.quantity || item?.quantity}{" "}
                        {/* {drugStrengthUnits.find(
                          (unit) =>
                            unit.id ===
                            (item?.doctorPrescription?.drugStrengthUnit ||
                              item?.drugStrengthUnit)
                        )?.name || "N/A"} */}
                      </span>
                    </div>
                  ))}
                ...
                <td>
                  {medications.map((item) => (
                    ...
                    >
                      <span>
                        {item?.doctorPrescription?.duration || item?.duration}{" "}
                        days
                      </span>
                    </div>
                  ))}
          ...
          <button onClick={() => setAdd(true)} className="submit-btn m-t-10">
              ...
              <div className="flex">
                <div>
                  <div className="w-100 flex m-t-20">
                    ...
                    <TagInputs
                      label="Nurse"
                      value={currentNote?.administeredBy}
                      readOnly={true}
                    />
                  </div>
                </div>
        ...
        <div className="m-t-20">
          ...
          {showVitalsSection && (
            <div className="card m-t-10">
              <VitalsTreat treatment={treatment} />{" "}
              {/* Render the Vitals component */}
            </div>
          )}
        </div>
          ...
          <div className="m-t-20 m-b-20">
            ...
            {documentArray.length === 0 ? (
              <div className="col-5">
                <button
                  onClick={addConsent}
                  className="rounded-btn m-t-20 col-3"
                >
                  Attach file
                </button>
              </div>
          ...
          <div className="flex gap-8">
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                key={`page-${i + 1}`}
                className={`pagination-btn ${
                  currentPage === i + 1 ? "bg-green text-white" : ""
                }`}
