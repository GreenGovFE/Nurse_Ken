...
function SendForVitalAsSpecialist({ closeModal }) {
  ...
  const sendForVital = async () => {
    ...
    let payloadToSend = {
      ...
      nurseId: userInfo?.employeeId || nurse?.value || 0,
      serviceId: service?.value || 0,
      categoryId: category?.value || 0,
    };
    console.log("Sending for vital with payload =>", {
      ...payloadToSend,
    });
    // return;

    const url = `/Appointment/checkin-patient-specialist`;

    setLoading(true);
    try {
      const res = await post(url, {...payloadToSend
        // ...payload,
        // patientId,
        // appointmentId: patientInfo?.appointmentId || 0,
      });
      ...
      if (res?.message == "successfully checked in") {
        notification({
          message: "successfully sent patient for vital",
          type: "success",
        });
  ...
  return (
    <div className="overlay">
      ...
      <div className="modal-contents">
        ...
        <div className="w-100">
          <TagInputs
            onChange={(e) => handleSelectChange(e, "quantity")}
        ...
        <div className="p-20">
          ...
          <div className="flex flex-v-center m-t-20">
            ...
            />
            <label htmlFor="isEmergency" className="m-l-10">
              Is Emergency
            </label>
          </div>
          <button
            className="submit-btn m-t-20 w-100"
            onClick={sendForVital}
            disabled={loading}
          >
            Submit
          </button>
...
export default SendForVitalAsSpecialist;
